...
import { MainlayerError, MainlayerRouterOptions } from './types';

/**
 * Creates an Express Router pre-wired with Mainlayer payment routes.
 */
export function createMainlayerRouter(
  options: MainlayerRouterOptions
): Router {
   ...
   */
  router.get('/mainlayer/discover', async (_req: Request, res: Response) => {
    try {
   ...
   * Response:     PayResult
   */
  router.post('/mainlayer/pay', async (req: Request, res: Response) => {
    const { resource_id, payer_wallet } = req.body as {
      resource_id?: string;
      payer_wallet?: string;
    };
   ...
   */
  router.get(
    '/mainlayer/access/:resourceId',
    async (req: Request, res: Response) => {
      const { resourceId } = req.params as { resourceId: string };
      const payerWallet =
        (req.query['payer_wallet'] as string | undefined) ??
      ...
      if (!payerWallet) {
        res.status(400).json({
          error: 'validation_error',
          message:
            'payer_wallet query parameter (or x-wallet-address header) is required.',
        });
        return;
      }
...
function handleRouterError(err: unknown, res: Response): void {
